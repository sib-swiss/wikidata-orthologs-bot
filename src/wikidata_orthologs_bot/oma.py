...
if __name__ == "__main__":
    ...
    for filename in tqdm(os.listdir(data_folder), desc="Processing OMA orthologs files", unit="file"):
        if filename.endswith(".csv"):
            ...
            missing_gene1 = set(df["gene1"].astype(str)) - set(ensembl_mapper.keys())
            missing_gene2 = set(df["gene2"].astype(str)) - set(ensembl_mapper.keys())
            combined_errors["gene_not_found"].update(missing_gene1)
            combined_errors["gene_not_found"].update(missing_gene2)

            # Remove all rows for which the gene1 or gene2 are not in the ensembl_mapper
            df = df[(df["gene1"].astype(str).isin(ensembl_mapper)) & (df["gene2"].astype(str).isin(ensembl_mapper))]
